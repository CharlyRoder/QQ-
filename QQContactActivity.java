...
import com.example.networks.adapter.FriendExpandAdapter;
import com.example.networks.asyctask.QueryFriendTask;
import com.example.networks.bean.Friend;
import com.example.networks.bean.FriendGroup;
import com.example.networks.bean.FriendGroupClass;
...
public class QQContactActivity extends AppCompatActivity implements
    ...
    public static Handler jsonHandler =new Handler(Looper.myLooper()){
        @Override
        public void handleMessage(@NonNull Message msg) {
            String resp_data_content= (String) msg.obj;
            FriendGroupClass friendGroupClass=new Gson().fromJson(resp_data_content,FriendGroupClass.class);
            mGroupList=friendGroupClass.group_list;
            Log.d("allgroups",mGroupList.get(0).group_title);
            showAllFriend();
        }
    };
...
//            }
            // 下面利用gson库自动解析json串
            FriendGroupClass packageResp = new Gson().fromJson(resp, FriendGroupClass.class);
